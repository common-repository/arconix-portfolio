 ...
 *
 * @since 1.0.0
 * @package arconix-portfolio/public
 */

/**
 * Define the portfolio shortcode attributes.
 */
class Arconix_Portfolio {
	...
	public function loop( $args, $echo = false ) {
		...
		if ( 'below' !== $args['title'] ) {
			$args['title'] = 'above'; // For backwards compatibility with "yes" and built-in data check.
		}
	...
	public function item( $args ) {
		...
		if ( 'above' === $args['title'] ) {
			$s .= $this->get_portfolio_title( $args['title_link'], $args['link'], $args['full'] );
		}
